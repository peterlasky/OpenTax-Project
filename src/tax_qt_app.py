...
try:
    from pdf_preview import F1040PdfPreviewEngine
except Exception:  # pragma: no cover - runtime dependency availability varies
    F1040PdfPreviewEngine = None
...
TABLE_HEADERS = ["Cell ID", "Description", "Value", "Format", "Equation", "Override?", "Required Rule"]
...
ONE_HOT_PREFIXES = ("filing_status_",)
...
class TaxSheetEditor(QMainWindow):
    def __init__(self, json_path: Path | None = None) -> None:
        ...
        self.pdf_preview_engine = (
            F1040PdfPreviewEngine(self._project_root())
            if F1040PdfPreviewEngine is not None
            else None
        )
    ...
    def _preview_source_path(self) -> Path:
        return self._project_root() / "forms-instructions-and-publications" / "forms" / "f1040.pdf"

    def _current_preview_form_id(self) -> str | None:
    ...
    def _resolve_pdf_mapping_source(self, source: str) -> Any:
        expr = source.strip()
        if not expr:
            return None
        return self._evaluate_equation(PDF_PREVIEW_FORM_ID, expr, set())
    ...
    def _update_pdf_preview(self) -> None:
        if self._current_preview_form_id() != PDF_PREVIEW_FORM_ID:
            self._show_pdf_placeholder("PDF preview is blank until Form 1040 is selected.")
            return
        if not self.data or not self.jurisdiction_key:
            self._show_pdf_placeholder("Start a return to preview Form 1040.")
            return
        raw_pdf = self._preview_source_path()
        if self.pdf_preview_engine is None:
            if raw_pdf.is_file():
                self._load_pdf_into_view(raw_pdf, "Raw Form 1040 preview")
            else:
                self._show_pdf_placeholder("PDF preview engine is unavailable in this runtime.")
            return
        if not self.pdf_preview_engine.available():
            if raw_pdf.is_file():
                self._load_pdf_into_view(
                    raw_pdf,
                    "Raw Form 1040 preview (install PDF preview dependencies for filled previews)",
                )
            else:
        ...
        try:
            preview_path = self.pdf_preview_engine.render_preview(
                resolve_source=self._resolve_pdf_mapping_source
            )
        except Exception as exc:
            if raw_pdf.is_file():
                self._load_pdf_into_view(raw_pdf, f"Preview fallback: raw Form 1040 ({exc})")
            else:
                self._show_pdf_placeholder(f"Could not render Form 1040 preview: {exc}")
            return

        self._load_pdf_into_view(preview_path, "Filled Form 1040 preview")
    ...
    def _auto_visible_sheet_ids(self) -> list[str]:
        ...
        expanded_form_ids = set(base_form_ids) | active_form_ids
        queue = list(active_form_ids)
        while queue:
        ...
        sheet_ids: list[str] = []
        for form_id, form_data in self._available_forms():
            if form_id in expanded_form_ids:
