...
- `f1040_Federal_Info_Worksheet.spouse_age_1_1_2026`
  ...
  Resolution:
  - fixed `src/main.py` so self-reference rewriting skips quoted strings
  Broader impact:
...
- evaluator numeric-literal handling
  ...
  Resolution:
  - fixed `src/main.py` so only bare numeric tokens that are actual cell IDs on the current form are rewritten as self references
...
- when an instruction introduces a cap, threshold phaseout, or date-based branch, add it to `forms-instructions-and-publications/instruction-summaries/` and check whether the current model encodes the full rule, a partial rule, or an explicit override-only gap.
- 